...
void Pop_double(vector_double *vec) {
  if (!IsEmpty_double(vec)) {
    ...
    fprintf(stderr,
            "[ERROR] : Cannot perform \"Pop\" for an empty vector_double");
    exit(EXIT_FAILURE);
  }
}
...
DATA Back_double(vector_double *vec) {
  if (vec->size > 0)
    ...
    fprintf(stderr,
            "[ERROR] : Cannot provide \"Back\" for an empty vector_double");
    exit(EXIT_FAILURE);
  }
}
...
DATA Front_double(vector_double *vec) {
  if (vec->size > 0)
    ...
    fprintf(stderr,
            "[ERROR] : Cannot provide \"Front\" for an empty vector_double");
    exit(EXIT_FAILURE);
  }
}
