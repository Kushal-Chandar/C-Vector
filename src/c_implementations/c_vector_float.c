...
void Pop_float(vector_float *vec) {
    if (!IsEmpty_float(vec)) {
        ...
        fprintf(stderr, "[ERROR] : Cannot perform \"Pop\" for an empty vector_float");
        exit(EXIT_FAILURE);
    }
}
...
DATA Back_float(vector_float *vec) {
    if (vec->size > 0)
        ...
        fprintf(stderr, "[ERROR] : Cannot provide \"Back\" for an empty vector_float");
        exit(EXIT_FAILURE);
    }
}
...
DATA Front_float(vector_float *vec) {
    if (vec->size > 0)
        ...
        fprintf(stderr, "[ERROR] : Cannot provide \"Front\" for an empty vector_float");
        exit(EXIT_FAILURE);
    }
}
