...
void Pop_char(vector_char *vec) {
    if (!IsEmpty_char(vec)) {
        ...
        fprintf(stderr, "[ERROR] : Cannot perform \"Pop\" for an empty vector_char");
        exit(EXIT_FAILURE);
    }
}
...
DATA Back_char(vector_char *vec) {
    if (vec->size > 0)
        ...
        fprintf(stderr, "[ERROR] : Cannot provide \"Back\" for an empty vector_char");
        exit(EXIT_FAILURE);
    }
}
...
DATA Front_char(vector_char *vec) {
    if (vec->size > 0)
        ...
        fprintf(stderr, "[ERROR] : Cannot provide \"Front\" for an empty vector_char");
        exit(EXIT_FAILURE);
    }
}
