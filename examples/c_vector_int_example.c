...
int main(int argc, char const *argv[]) {
    ...
    printf("size: %llu\ncapacity: %llu\naddress:%p\n", vec_int_b.size, vec_int_b.capacity, vec_int_b.data);

    printf("\n\n");

    // Reserving the Size of Vector
    printf("Reserve : vec_int_a to 5\n");  // increasing capacity and size
    ReserveVectorSize_int(&vec_int_a, 5);  // now a can store upto 5 elements without doubling its size
    printf("size: %llu\ncapacity: %llu\naddress:%p\n", vec_int_a.size, vec_int_a.capacity, vec_int_a.data);

    printf("Reserve : vec_int_b to 5\n");  // shrinking capacity and size
    ReserveVectorSize_int(&vec_int_b, 5);  // now a can store upto 5 elements without doubling its size
    printf("size: %llu\ncapacity: %llu\naddress:%p\n", vec_int_b.size, vec_int_b.capacity, vec_int_b.data);
    ...
    for (size_t i = 0; i < vec_int_b.size; i++)
        printf("%d ", vec_int_b.data[i]);
